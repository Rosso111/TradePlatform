            ...
            while sim_date <= run.end_date:
                ...
                for signal in signals:
                    if signal['action'] == 'BUY':
                        _log_decision(run, signal, sim_date, executed=False)

                        if signal['stock_id'] in open_position_stock_ids:
                            continue
                        if len(open_position_stock_ids) >= config.MAX_POSITIONS:
                            continue
                        ...
                        commission = calc_commission(budget_eur)
                        spread = calc_spread_cost(budget_eur)
                        ...
                        if position:
                            cash_eur += _close_position(run, position, sim_date, signal['reason'])
...
def _close_position(run: SimulationRun, position: SimulationPosition, sim_date, reason: str) -> float:
...
def _log_decision(run: SimulationRun, signal: dict, sim_date, executed: bool):
